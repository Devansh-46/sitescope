...
// Uses waitUntil() so Vercel doesn't kill the process after the response is sent
...
export async function POST(request: NextRequest) {
  try {
    ...
    // waitUntil keeps the Vercel serverless function alive until the pipeline finishes
    // Without this, Vercel freezes the process as soon as the response is sent
    waitUntil(runAnalysisPipeline(report.id, url));
